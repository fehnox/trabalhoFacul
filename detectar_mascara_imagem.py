...
def detectar_mascara_imagem(caminho_imagem):
    ...
    print("  💡 Pressione qualquer tecla para fechar a janela ou 's' para salvar a imagem anotada")
    key = cv2.waitKey(0)
    if key == ord('s'):
        nome_saida = f"anotada_{os.path.basename(caminho_imagem)}"
        cv2.imwrite(nome_saida, frame)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if opcao == '1':
            ...
            print("\nℹ️ INSTRUÇÕES:")
            print("- O sistema aceita imagens nos formatos: JPG, PNG, BMP, TIFF")
            print("- Para salvar a imagem anotada, pressione 's' na janela de visualização")
            print("- Para fechar a janela, pressione qualquer outra tecla")
            print("- As mensagens exibidas indicam o status do uso de máscara detectado")
            print("- Se aparecer 'Nenhuma pessoa detectada', verifique a imagem escolhida")
            print("- Para dúvidas sobre o modelo, consulte o README.md do projeto")
